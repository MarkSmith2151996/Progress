...
// Write context
function writeContext(context: any) {
  ensureDataFile();
...
${Object.entries(context.goals).map(([key, g]: [string, any]) => {
  if (g.type === 'number') return `- ${g.name}: ${g.current}/${g.target} (started at ${g.start})`;
  if (g.type === 'cumulative') return `- ${g.name}: $${g.current}/$${g.target}`;
  if (g.type === 'time') return `- ${g.name}: ${g.total} hours total`;
  if (g.type === 'checklist') return `- ${g.name}: ${g.items?.length || 0} items`;
  return `- ${g.name}`;
}).join('\n')}
...
function buildParsePrompt(context: any): string {
  return `You are a data parser. Extract structured data from natural language input.

USER'S GOALS:
${Object.entries(context.goals).map(([key, g]: [string, any]) => `- ${key}: ${g.name} (${g.type})`).join('\n')}
...
// Update context
ipcMain.handle('update-context', async (_, updates: any) => {
...
ipcMain.handle('parse-input', async (_, input: string) => {
  try {
    const context = readContext();
...
ipcMain.handle('apply-parsed-items', async (_, items: any[]) => {
  try {
    const context = readContext();
